...
const int COMPLEX_DATA_TYPE_FRONT = 1;
const int COMPLEX_DATA_TYPE_BACK = 2;
const int FOUR_VEC_GET_LAST_TYPE = 3;
...
void main() {
    ...
    if (heightDataType == COMPLEX_DATA_TYPE_FRONT) {
        complex psi = texture2D(heightTex, UV).xy;
        height = psi.x*psi.x + psi.y*psi.y;
    } else if (heightDataType == COMPLEX_DATA_TYPE_BACK) {
        complex psi = texture2D(heightTex, UV).zw;
        height = psi.x*psi.x + psi.y*psi.y;
    } else if (heightDataType == FOUR_VEC_GET_LAST_TYPE) {
        height = texture2D(heightTex, UV).w;
    }
