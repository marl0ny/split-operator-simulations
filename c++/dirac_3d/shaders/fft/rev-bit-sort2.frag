...
/* Older versions of GLSL do not support for loops.
 This long function with repetitious if statements is used to reverse 
 bit sort a finite-sized input texture with power-of-two dimensions without
 the aid of for loops. For more modern versions of GLSL a much shorter
 implementation of reverse bit sorting is used instead, one that utilizes
 for loops. */
