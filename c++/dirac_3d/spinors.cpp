...
BiSpinor get_spinor_plane_wave(
    SimParams sim_params, Vec3 p, std::vector<complex> e, 
    int representation) {
    float m = sim_params.m;
    float c = sim_params.c;
    Spinor up = get_spin_up_state(p.normalized(), p.length());
    Spinor down = get_spin_down_state(p.normalized(), p.length());
