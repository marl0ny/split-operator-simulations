...
static Quad &fft_iter_hypercube(
    uint32_t program,
    fft4d::QuadTemps &iter_quads,
    bool is_inverse, IVec4 texel_dimensions
) {
    ...
    for (int block_size = 2; block_size <= size; block_size *= 2) {
        ...
        iter_quads.ind[indices[1]].draw(
            program,
            {
                ...
                {"scale", {scale}},
                {"size", {float(size)}},
                {"texelDimensions4D", {texel_dimensions}}
            }
        );
...
void fft4d::fft(Quad &dst, const Quad &src,
    ...
    dst = fft_iter_hypercube(
        fft_iter_program, iter_quads, false, texel_dimensions);
}

void fft4d::ifft(Quad &dst, const Quad &src,
         QuadTemps &iter_quads,
         Programs fft_programs,
         IVec4 texel_dimensions) {
    uint32_t rev_bit_sort2_program = fft_programs.rev_bit_sort2;
