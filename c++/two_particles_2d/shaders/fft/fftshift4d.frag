...
void main() {
    ...
    vec4 coord4DShifted = vec4(
        (coord4D[0] < 0.5)? coord4D[0] + 0.5: coord4D[0] - 0.5,
        (coord4D[1] < 0.5)? coord4D[1] + 0.5: coord4D[1] - 0.5,
        (coord4D[2] < 0.5)? coord4D[2] + 0.5: coord4D[2] - 0.5,
        (coord4D[3] < 0.5)? coord4D[3] + 0.5: coord4D[3] - 0.5);
    vec2 coord2DShifted = to2DTextureCoordinates(coord4DShifted);
    fragColor = texture2D(tex, coord2DShifted);
}
