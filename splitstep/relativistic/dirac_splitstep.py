...
class DiracSplitStepMethod(SplitStepMethod):
    ...
    def set_timestep(self, timestep: Union[float, np.complex128]) -> None:
        ...
        if self._vector_potential:
            ...
            exp_V = [[1.0*np.exp(-0.25*1.0j*V*dt), 0, 0, 0], 
                    [0, 1.0*np.exp(-0.25*1.0j*V*dt), 0, 0], 
                    [0, 0, 1.0*np.exp(-0.25*1.0j*V*dt), 0], 
                    [0, 0, 0, 1.0*np.exp(-0.25*1.0j*V*dt)]]
            self._exp_V = exp_V
